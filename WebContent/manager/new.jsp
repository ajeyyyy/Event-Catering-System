...
<body>
		...
		<tr>
			<td>
				<form name="companyForm" action="<c:url value='/EventController?updateEvent' />" method="post">
					...
					<input name="action" value="updateEvent" type="hidden"/> <input name="id" value="<c:out value="${event.id}" />" type="hidden">
					<input type="submit" value="Update event" onclick="return checkIt()"/>
				</form>
			</td>
		</tr>
	</table>
	...
	<span class="success"> <c:out value='${userErrorMsgs.userDeleteSuccess}' /></span>
	<table border="1" class="">
		...
		<c:forEach items="${AssignedStaff}" var="item" varStatus="status">
			<tr>
				...
				<td><a href="<c:url value='/EventController?action=deleteEvents&username=${item.username}&eventId=${event.id}' />" onclick="return confirm('Are you sure you want to remove this user?')">Delete</a></td>
			</tr>
		</c:forEach>
	...
	<form action="<c:url value='/EventController?assignEvents' />" method="post">
		...
		<input type="submit" value="Submit" onclick="return checkIt()">
	</form>
</body>
